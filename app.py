...
if "animation_jouee" not in st.session_state: st.session_state.animation_jouee = False
...
if st.session_state.page == "accueil":
    ...
    with st.container():
        ...
        # Sélection de l'avatar et du pseudo
        col1, col2 = st.columns([1, 4])
        with col1:
            avatar = st.selectbox("Avatar", LISTE_AVATARS, label_visibility="collapsed")
        with col2:
            pseudo = st.text_input("Ton Pseudo", max_chars=12, placeholder="Ex: Agent 007", label_visibility="collapsed")
...
# ==========================================
    ...
    if st.session_state.est_createur:
        ...
        if st.button("🚀 Lancer la partie", type="primary", use_container_width=True):
            try:
                supabase.table("salons").update({"vainqueur": None, "cibles_egalite": None}).eq("code_salon", st.session_state.code_salon).execute()
            except: pass
...
# ==========================================
    ...
    # --- PHASE Z : ÉCRAN DE VICTOIRE (AVEC ANIMATIONS) ---
    if statut_actuel == "victoire" or vainqueur_actuel:
        
        # Déclenchement de l'animation une seule fois
    ...
    # ----------------------------------------
        ...
        if len(a_vote) == len(joueurs_en_vie) and len(joueurs_en_vie) > 0:
            ...
            if len(top_votes) > 1 and top_votes[0][1] == top_votes[1][1]:
                ...
                # Remise à zéro des votes et passage en phase d'égalité
    ...
    # ----------------------------------------
    # PHASE B Bis : LE TIE-BREAKER (ÉGALITÉ)
    # ----------------------------------------
        ...
        # Logique d'exécution du second vote
        if len(a_vote) == len(joueurs_en_vie) and len(joueurs_en_vie) > 0:
            votes = [j["vote_contre"] for j in a_vote]
            compte = Counter(votes)
            top_votes = compte.most_common()
            
            # Même s'il y a encore égalité, on élimine le premier trouvé pour éviter une boucle infinie
            joueur_elimine = top_votes[0][0]
        ...
        # Interface de vote restreinte
        if mon_profil and not mon_profil["est_elimine"]:
            if mon_profil["vote_contre"]:
                st.success("✅ Tu as revoté !")
            else:
                st.write("Choisis qui doit être éliminé :")
                st.markdown('<div class="vote-btn">', unsafe_allow_html=True)
                for cible in cibles_list:
                    # Dans le départage, on a même le droit de voter contre soi-même !
